...
export default defineCommand({
  meta: {
    name: 'export',
    description: 'Export design-specs tokens to other formats. `css-tailwind` emits Tailwind v4 CSS @theme; `json-tailwind` emits Tailwind v3 theme.extend JSON; `tailwind` is an alias for `json-tailwind`; `dtcg` emits W3C Design Tokens.',
  },
  args: {
    file: {
      type: 'positional',
      description: 'Path to design-specs (use "-" for stdin)',
      required: true,
    },
    format: {
      type: 'string',
